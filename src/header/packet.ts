...
export class Receiver {
 ...
 constructor (
   ...
   const maybeTheList = this.decoder.decode(theListPacked)
 ...
 public decoder: D.Decoder<unknown, PacketList.Value> = {
   decode: (a: unknown) => {
     return pipe(
       ...
       // 1. Deserialize the header bytes from the message stream using
       //    MessagePack.
       ...
       E.chain(value => {
         ...
         // 3. Deserialize the bytes from #1 again using MessagePack to give the
         // header list.
         const maybeTheList = pipe(
         ...
         const ephemeralSharedSecret = NaCl.box.before(
           ephemeralPublicKey,
         ...
         // the nonce saltpack_recipsbXXXXXXXX. XXXXXXXX is 8-byte big-endian
         // unsigned recipient index, where the first recipient is index 0.
         // Successfully opening one gives the payload key.
         const [payloadKey, index] = recipientsList.reduce((acc:[ Bytes.Value|null, number ], item:RecipientsList.Item, index:number) => {
           if (!acc[0]) {
             const attempt = NaCl.box.open.after(
               item[1],
               Nonce.indexed(RecipientPublicKey.NONCE_PREFIX, index),
         ...
         const senderPublicKey = NaCl.secretbox.open(
           senderSecretBox,
           SenderKeyNonce.NONCE,
           this.payloadKey()
         )
